"""GitHub Agent Runtime - Dual-mode client/agent communication.
...
@app.entrypoint
async def strands_agent_github(payload: Dict[str, Any]):
    """GitHub Agent entrypoint with dual-mode support.
    ...
    if not user_input:
        ...
        if mode == ResponseMode.AGENT:
            yield create_response(False, error_msg, agent_type="github").to_dict()
        else:
            yield error_msg
        return
