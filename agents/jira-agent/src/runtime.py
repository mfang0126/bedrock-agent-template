...
async def strands_agent_jira(payload):
    ...
    # Create Jira agent with auth injection
    agent = create_jira_agent(auth)

    # Initialize Jira OAuth - this will trigger OAuth flow if no token exists
    print("🔐 Initializing Jira authentication...")
    ...
    try:
        ...
        print(f"⚠️ Jira authentication pending or failed: {e}")
        ...
        if hasattr(auth, 'get_pending_oauth_url'):
            ...
            if pending_url and not oauth_url_received:
                oauth_message = create_oauth_message(pending_url, "Jira")
                # Yield OAuth URL
                yield format_client_text(oauth_message)
                return

    # OAuth successful, proceed with agent streaming
    print("🚀 Processing Jira request with streaming...")

    try:
        # Process with Jira agent using streaming
        ...
        error_message = f"❌ Error processing Jira request: {str(e)}"
